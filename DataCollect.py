...
while True:
    print(f"Press USER button {run_idx}")
    while btn.value():
        sleep_ms(20)
    print(f"button pressed {run_idx}")

    filename = f"imu_pid_gyroZ_{date_str}_run{run_idx}.csv"
    f = open(filename, "w")
    f.write("time_ms,acc_x,acc_y,acc_z,gyro_x,gyro_y,gyro_z,yaw_error\n")
    f.flush()

    pid = PID(PID_KP)
    start = ticks_ms()
    end = start + LOG_DURATION_MS
    ...
    while ticks_diff(end, ticks_ms()) > 0:
        ...
        print(f"ω_z = {gz:.3f} rad/s | yaw_err = {yaw_error:.3f} | PID_corr = {correction:.1f} | L_PWM = {left_pwm:.0f}, R_PWM = {right_pwm:.0f}")
        line = f"{t},{ax:.3f},{ay:.3f},{az:.3f},{gx:.3f},{gy:.3f},{gz:.3f},{yaw_error:.3f}"
        f.write(line + "\n")
        f.flush()
    ...
    f.close()
    print(f" {run_idx} complete. Saved: {filename}")
    run_idx += 1
